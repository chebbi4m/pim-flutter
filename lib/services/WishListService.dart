...
class WishListService {
  ...
  final WishList wishedProduct = WishList(
    ...
    productId: product.id, // Assuming the product ID is accessible from the 'Product' object
    statut: false, // Assuming the default status is "false" for new wished products
  );
  ...
  try {
    ...
    if (response.statusCode == 201) {
      ...
      print('Failed to add wished product - Status Code: ${response.statusCode}, Response Body: ${response.body}');
  ...
  static Future<void> deleteWishedProduct(String childId, String productId) async {
    try {
      final response = await http.delete(Uri.parse('$baseUrl/wishlist/delete/$childId/$productId'));
      if (response.statusCode == 200) {
  ...
  static Future<WishList> updateWishedProduct(String childId, String productId) async {
    final response = await http.put(Uri.parse('$baseUrl/wishlist/edit/$childId/$productId'));
    if (response.statusCode == 200) {
      final dynamic responseData = json.decode(response.body);
      return WishList.fromJson(responseData);
    } else {
  ...
  static Future<WishList?> getWishedProductById(String childId, String productId) async {
    final response = await http.get(Uri.parse('$baseUrl/wishlist/$childId/$productId'));
    if (response.statusCode == 200) {
      final dynamic responseData = json.decode(response.body);
      return WishList.fromJson(responseData);
    } else if (response.statusCode == 404) {
  ...
  static Future<List<Product>> getAllProductDetails(String childId) async {
    final response = await http.get(Uri.parse('$baseUrl/wishlist/allProducts/$childId'));
    if (response.statusCode == 200) {
      final List<dynamic> responseData = json.decode(response.body);
